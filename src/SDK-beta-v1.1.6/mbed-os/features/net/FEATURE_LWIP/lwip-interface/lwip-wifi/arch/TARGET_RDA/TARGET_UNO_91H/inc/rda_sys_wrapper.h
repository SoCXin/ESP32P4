 ...
 * @param[in] : func(callback)/data(pass to func)
 * @param[out]:
...
void* rda_mail_create(unsigned int num, unsigned int size);

/**
 * @brief     : get a msg from mail
 * @param[in] : name of mail/message/msg size/wait time
 * @param[out]:
 * @return    : return ERR or NO_ERR
 */
int rda_mail_get(void* mailq, const void* msg, unsigned int size, unsigned long wait);
...
int rda_mail_put(void* mailq, const void* msg, unsigned int size, unsigned long wait);
#endif
